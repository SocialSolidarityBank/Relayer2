...
| D2 | **2026-09-15 확정: 앱은 맥미니 + Cloudflare Tunnel, DB 는 Supabase Pro 서울 리전.** 추가 비용 0원 | Vercel Hobby 는 약관이 내부 업무 도구를 금지한다(Pro 는 월 $20), Fly Managed PG 는 월 $38. 맥미니가 상시 켜져 있고 Cloudflare·Supabase 는 이미 쓰는 곳이라 업체를 늘리지 않는다. 서울 리전이라 실데이터로 넘어갈 때 옮기지 않아도 된다 |
| D3 | `org_id` 안 넣음 | A3 |
