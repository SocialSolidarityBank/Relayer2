...
| D2 | **2026-09-15 확정: 앱은 맥(Tailscale Funnel), DB 는 Supabase Pro 서울 리전(`relayer2` 전용 프로젝트).** 추가 비용 0원. Cloudflare Tunnel 은 계정에 등록된 도메인이 없어 DNS 를 못 붙였다 | Vercel Hobby 는 약관이 내부 업무 도구를 금지한다(Pro 는 월 $20), Fly Managed PG 는 월 $38. 맥미니가 상시 켜져 있고 Cloudflare·Supabase 는 이미 쓰는 곳이라 업체를 늘리지 않는다. 서울 리전이라 실데이터로 넘어갈 때 옮기지 않아도 된다 |
| D3 | `org_id` 안 넣음 | A3 |
...
| 인테이크 작성하기 | 사용자 피드백의 9개 구획(SPEC §9). 실제 일시·방식, 수급자 여부, 선호 방식, 신청 사유, 지원·강점, 전체 목표, 과제·질문 | `promise`·`question`. 과거 fact·judgment와 숨겨진 응답은 보존 |
| 상담 일정 등록 | `sessions`에 `status=planned` 행 1개(일시·방법·**메모**) | — (메모는 카드가 아니다, 요구 4) |
| 상담 기록하기 | 5구획(오늘 상담 내용 필수 / 수행할 과제 / 다음에 물어볼 것 / 실무자 의견 / 다음 상담 목표). 열린 카드는 보조 레일 + 결과 버튼 | `promise`·`question`·`judgment` + 미처리 카드 `unchecked`. 과거 fact 카드는 보존 |
...
- 당사자 정보·상담 종결 **완료 2026-09-15**: 탭 4(회차별 요약·15초 다시보기·목표·정보)와 종결 화면(화면 7 → 9, API 8 → 10). P2로 미뤄 뒀지만 `case_closures` 표가 이미 있어 화면만 붙이면 됐다. 회차별 요약에서 마지막 상담과 종결이 두 항목으로 보이고, 종결은 회차 번호를 받지 않는다.
- 종결 상담 **완료 2026-09-15**(요구 5): `sessions.is_closing`. 일정에서 미리 고르거나 기록에서 그 자리에서 고르고, **저장 성공 뒤에만** 종결 화면으로 간다. 실패하면 사례를 닫지 않는다.
- **베타 0.1.0 마감 2026-09-15**(태그 `v0.1.0`). 화면 9 · API 11 · 스키마 10표 + 마이그레이션 5 · 단위 11 · E2E 7. 등록 → 인테이크 → 일정 → 기록 → 15초 다시보기 → 당사자 정보 → 종결까지 한 사례가 끝까지 돈다. 인테이크·회차 고쳐 쓰기, 넓은 화면 두 칸까지 포함한다.
- 남은 것: 사내 임시 배포(D2 미정이라 로컬 우선), **관문 2 실측**. 관문 2 는 사람이 필요하므로 코드는 P1 로 넘어간다.

### 베타 밖 (각각 별도 Seed)

| 단계 | 내용 | 시작 조건 |
|---|---|---|
| P1 실데이터 게이트 | ~~동의 2영역·철회·문안 해시~~ **2026-09-15 완료**(`SPEC.md` §10), ~~`audit_log`~~ **완료**(§11), ~~자유 글 암호화~~ **완료**(§12), ~~백업·복구~~ **완료**(§13), 국내 리전 배포(D2) | 실제 당사자 자료를 넣기로 할 때 |
| P2 결과물 확장 | ~~당사자 정보 탭 4~~·~~상담 종결~~·~~일정 화면~~ 완료(0.1.0), ~~당사자 열람 링크+코드~~ **2026-09-15 완료**(`SPEC.md` §14). 남은 것: 당사자 초대 화면 | 관문 2 통과 후 |
| P3 AI 텍스트 경로 | **2026-09-15 완료**(`SPEC.md` §15): 동의(외부 LLM·국외 처리) → 가림 처리 → 호출 → 초안 → 승인. 남은 것: 불일치 2종(음성·수기 / 회차간)은 P4 STT 뒤 | P1 완료 후 |
| P4 STT | 동의 3영역 확장 **완료**(`SPEC.md` §16). 남은 것: 녹음 → 전사 경로, 불일치 2종. 엔진은 정본이 `azure` 로 못박음 | S13 자격 통과 후 |
