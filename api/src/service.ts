...
import type { AiEvidence, Assignee, Card, CardOutcome, FactChange, Session, SupportCase } from './domain/types.ts';
...
export type ApprovedSummary = { summary: string; changes: string[]; fact_changes: FactChange[]; evidence: AiEvidence[] };
...
async function approvedSummaries(sessionIds: number[]): Promise<Record<number, ApprovedSummary>> {
  ...
  const rows = await sql<Array<{ session_id: number; status: 'draft' | 'approved' } & ApprovedSummary>>`
    select distinct on (session_id) session_id, status, summary, changes, fact_changes, evidence
    from ai_drafts where session_id in ${sql(sessionIds)} order by session_id, id desc`;
  return Object.fromEntries(
    rows
      .filter((r) => r.status === 'approved')
      .map((r) => [r.session_id, { summary: r.summary, changes: r.changes, fact_changes: r.fact_changes, evidence: r.evidence }]),
  );
}
