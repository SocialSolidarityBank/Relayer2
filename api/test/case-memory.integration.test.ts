...
import { MEMORY_HEADER, MEMORY_MAX_CHARS, refreshCaseMemory, settleCaseMemory } from '../src/ai.ts';
...
describe.skipIf(!enabled)('사례 기억', () => {
  ...
  it('초안은 기억을 읽고 지난 회차 전량을 보내지 않는다', async () => {
    const { case_id, worker } = await caseWith(CONSENTS);
    await record(case_id, worker, 1);
    const s2 = await record(case_id, worker, 2);
    sent = [];

    expect((await req(`/sessions/${s2}/draft`, worker, 'POST', {})).status).toBe(200);
    expect(memoryCalls()).toHaveLength(0);
    const [call] = draftCalls();
    expect(call.prompt.split(MEMORY_HEADER)).toHaveLength(2);
    expect(call.prompt).not.toContain('[지난 회차');
    expect(call.prompt).toContain('전체 목표: 월세 연체 해소');
    expect(call.body).not.toContain(REAL.name);
  });

  it('원본을 고치면 지워지고 다시 접히며, 초안은 어긋난 기억을 쓰지 않고 다시 만들고, 그것도 안 되면 503', async () => {
    const { case_id, worker } = await caseWith(CONSENTS);
    const s1 = await record(case_id, worker, 1);
    const s2 = await record(case_id, worker, 2);
    expect((await memoryRow(case_id))?.through_seq).toBe(1);
    ...
    // 2회차 초안은 "1회차까지" 를 원한다 — 2회차까지 접힌 기억은 쓰지 않고 다시 만든다.
    sent = [];
    expect((await req(`/sessions/${s2}/draft`, worker, 'POST', {})).status).toBe(200);
    expect(memoryCalls()).toHaveLength(1);
    expect(memoryCalls()[0].prompt).toContain(`[1회차 session_id=${s1}]`);
    expect(memoryCalls()[0].prompt).not.toContain(`session_id=${s2}]`);
    expect((await memoryRow(case_id))?.through_seq).toBe(1);
    expect((await lastMemoryAudit(case_id)).fields).toContain('trigger=draft_request');

    // 마지막 회차(2회차) 수정: 그 앞까지만 접힌다 — 2회차 초안이 바로 쓸 수 있게.
    sent = [];
    await revise(s2, worker, `${REAL.name} 월세 2개월. 고침.`);
    expect((await memoryRow(case_id))?.through_seq).toBe(1);
    expect(memoryCalls()[0].prompt).not.toContain(`session_id=${s2}]`);

    // 근거가 사례 밖을 가리키면 실패. 상한을 넘어도 실패. 둘 다 초안은 503 이고 초안 행이 늘지 않는다.
    const drafts = async () =>
      (await sql<Array<{ n: number }>>`select count(*)::int as n from ai_drafts where session_id = ${s2}`)[0].n;
    ...
    expect((await req(`/sessions/${s2}/draft`, worker, 'POST', {})).status).toBe(503);
    memoryReply = () => ({ text: '가'.repeat(MEMORY_MAX_CHARS + 1), refs: [] });
    expect((await req(`/sessions/${s2}/draft`, worker, 'POST', {})).status).toBe(503);
    expect(await memoryRow(case_id)).toBeNull();
    expect(await drafts()).toBe(draftsBefore);
  });

  it('초안 요청 본문은 회차 수에 비례해 자라지 않는다 (2회차 vs 8회차)', async () => {
    const { case_id, worker } = await caseWith(CONSENTS);
    const sessions: number[] = [];
    for (let seq = 1; seq <= 9; seq += 1) sessions.push(await record(case_id, worker, seq));
    const measure = async (sessionId: number) => {
      await sql`delete from case_memories where case_id = ${case_id}`;
      sent = [];
      expect((await req(`/sessions/${sessionId}/draft`, worker, 'POST', {})).status).toBe(200);
      const [call] = draftCalls();
      const [, block] = call.prompt.split(MEMORY_HEADER);
      expect(block).toBeDefined();
      expect(block.length).toBeLessThanOrEqual(MEMORY_MAX_CHARS + 1);
      expect(call.prompt).not.toContain('[지난 회차');
      return call.body.length;
    };
    const after2 = await measure(sessions[2]);
    const after8 = await measure(sessions[8]);
    expect(after8).toBeLessThanOrEqual(after2 + MEMORY_MAX_CHARS);
  });
