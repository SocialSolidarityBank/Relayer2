...
describe.skipIf(!enabled)('recording consent withdrawal', () => {
  it('deletes only that case\'s audio, keeps the transcript and other cases, and audits voice.withdraw', async () => {
    ...
    const [{ rel_path: p1 }] = await sql<Array<{ rel_path: string }>>`select rel_path from recordings where id = ${r1.id}`;
    const [{ rel_path: p2 }] = await sql<Array<{ rel_path: string }>>`select rel_path from recordings where id = ${r2.id}`;
    ...
    expect(await exists(p1)).toBe(true);
    ...
    expect(await exists(p1)).toBe(false);
    expect(await exists(p2)).toBe(true);
