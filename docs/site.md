...
## 사고 기록: 2026-09-18 운영 502

`./scripts/deploy-site.sh` 는 이름이 site 지만 **`main` 전체를 내보낸다.** 소개 페이지만
골라 보내지 않는다. 그날 문서 한 줄을 고쳐 배포했는데, 같은 `main` 에 다른 레인이
`@azure/identity` 의존성을 더해 둔 상태였다. 그 기기에는 그 꾸러미가 없어서 앱이 부팅에서
죽고 `relayer.kr` 이 약 2분 동안 502 였다.

고친 것 둘이다.

1. `package.json` 이나 `pnpm-lock.yaml` 이 바뀌면 받은 뒤 `pnpm install --frozen-lockfile` 을 돌린다.
2. 확인에서 200 이 아니면 앱 로그 마지막 20줄을 찍고, 되돌리는 명령까지 같이 보여 준다.

그래도 남는 사실은 하나다. **소개 페이지를 고치려고 누른 배포가 남의 코드까지 내보낸다.**
1단계가 바뀌는 파일 목록을 항상 먼저 보여 주는 이유가 이것이다. `api/` 나 `web/` 가 목록에
있으면 그 변경을 낼 준비가 된 것인지 먼저 확인한다.
