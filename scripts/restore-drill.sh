...
"$PSQL" "$ADMIN_URL" -q -c 'drop database if exists relayer_restore_drill' >/dev/null
"$PSQL" "$ADMIN_URL" -q -c 'create database relayer_restore_drill' >/dev/null
"$PSQL" "$DRILL_URL" -q -f "$file" >/dev/null 2>&1
echo "▸ 연습용 DB 로 복원"

ok=1
for t in participants support_cases sessions cards card_outcomes consent_events audit_log; do
  a="$(DATABASE_URL="$DATABASE_URL" "$PSQL" "$DATABASE_URL" -tA -c "select count(*) from $t")"
  b="$("$PSQL" "$DRILL_URL" -tA -c "select count(*) from $t")"
  mark="같음"
  if [ "$a" != "$b" ]; then mark="다름 ✗"; ok=0; fi
  printf "  %-16s 운영 %-6s 복구 %-6s %s\n" "$t" "$a" "$b" "$mark"
done

packed="$("$PSQL" "$DRILL_URL" -tA -c "select memo from sessions where memo is not null limit 1")"
if [ -n "$packed" ]; then
...
"$PSQL" "$ADMIN_URL" -q -c 'drop database relayer_restore_drill' >/dev/null
echo "▸ 연습용 DB 정리"
