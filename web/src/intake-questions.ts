...
/**
 * 마지막 구획 `참고 메모`(2026-09-18 Q 결정 D13). 앞의 아홉 구획 어디에도 안 들어가는 말을
 * 받는 **자유 글 한 칸**이다 — 선택이고, 비면 저장에서 키가 빠지지 않고 빈 문자열로 남는다
 * (다른 은퇴 문항과 같은 취급: 지우지 않는다).
 *
 * `INTAKE_GROUPS` 에 넣지 않은 까닭: 그 배열은 화면이 2~6구획으로 한 번에 그리는 자료라
 * 거기에 넣으면 `전체 상담 목표`·`수행할 과제`·`다음에 물어볼 것` 앞에 선다. 자리는 **맨 끝**이다.
 * 저장은 다른 문항과 똑같이 `detail_json` 의 고정 키 하나다(민감정보 처리 동의 범위 안 —
 * 자유 글이라 서버가 다른 인테이크 답과 같이 암호화한다).
 */
export const INTAKE_MEMO_QUESTION: IntakeQuestion = {
  key: 'reference_memo',
  label: '참고 메모',
  kind: 'textarea',
  hint: '예: 다음 상담 전에 확인할 기관 연락처, 당사자가 덧붙인 말',
};
