...
export function RecordScreen({ caseId }: { caseId: number }) {
  ...
  if (!session)
    return (
      <p className="empty">
        예정된 상담이 없어요. <a href={`#/cases/${caseId}/schedule`}>상담 일정 등록</a>이 먼저예요.
      </p>
    );
  const sessionId = session.id;
  const inPerson = session.method === 'in_person';
      ...
      <div className="wire-container rail-grid record-grid" data-grid="true">
        ...
        <main className="record-main">
          ...
          <FormActions>
            ...
            <Button variant="primary" disabled={!memo.trim() || saving} onClick={() => void save()}>
              {saving ? '저장 중…' : '저장'}
            </Button>
          </FormActions>
        </main>
